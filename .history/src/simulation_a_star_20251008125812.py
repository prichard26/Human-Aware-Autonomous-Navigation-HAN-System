...
def find_a_star_proba(cost_map, start, goal, max_iterations=1000000000):
    ...
    if (start[0] < 0 or start[0] >= cost_map.shape[0] or 
        start[1] < 0 or start[1] >= cost_map.shape[1]):
        print(f"Error: Start position {start} is out of bounds")
        return None
        
    if (goal[0] < 0 or goal[0] >= cost_map.shape[0] or 
        goal[1] < 0 or goal[1] >= cost_map.shape[1]):
