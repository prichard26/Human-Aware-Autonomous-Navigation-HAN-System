...
class HAANSystem:
    ...
    def _get_default_config(self) -> dict:
        ...
        return {
            ...
            'new_size': (1280, 1280),
            'start': (0, 0),
            'goal': (-20, 200),  # Closer goal position within bounds
            'simulation_time': 10000,  # Shorter simulation time
            'dt': 100,
            'grid_size': (int(1280*0.3), int(1000*0.3)),  # Proper grid size
    ...
    def process_image(self, image_path: str) -> dict:
        ...
        try:
            ...
            # Create obstacles for simulation with proper coordinate conversion
            ...
            for i, (horizontal_pos, depth, direction, speed) in enumerate(zip(horizontal_pos_in_cm, depth_values, directions_in_radians, speeds)):
                # Convert to simulation coordinates (matching original system)
                x_position = horizontal_pos * self.config['scale']
                y_position = depth * self.config['scale']
